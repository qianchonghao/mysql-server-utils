        ...
        for (Entry<String, Set<String>> entry : rawSource.entrySet()) {
            ...
            for (String tableName : tableNames) {
                executor.execute(() -> {
                    ...
                    int start = 0;
                    // diffInfo 不存储所有差异record，避免内存溢出。上限 = 500
                    for (int i = 0; start == i * LIMIT && diffInfo.getRecordsOnlyInSource().size() < LIMIT && diffInfo.getRecordsOnlyInTarget().size() < LIMIT; i++) {
                        List<Record> sourceRecords = DBQueryService.querySourceRecords(sourceDataSource, primaryColStructures, start);
                        List<Record> targetRecords = DBQueryService.queryTargetRecords(targetDataSource, primaryColStructures, start, sourceRecords);

                        Set<Record> sourceRecordSet = Sets.newHashSet();
                        sourceRecordSet.addAll(sourceRecords);

                        Set<Record> targetRecordSet = Sets.newHashSet();
                        targetRecordSet.addAll(targetRecords);

                        SetView<Record> onlyInSource = Sets.difference(sourceRecordSet, targetRecordSet);
                        SetView<Record> onlyInTarget = Sets.difference(targetRecordSet, sourceRecordSet);

                        if (targetRecordSet.size() != sourceRecordSet.size() || !onlyInSource.isEmpty() || !onlyInTarget.isEmpty()) {
                            diffInfo.getRecordsOnlyInSource().addAll(onlyInSource);
                            diffInfo.getRecordsOnlyInTarget().addAll(onlyInTarget);
                        }
                        start += sourceRecords.size();
                    }

                    if (!diffInfo.getRecordsOnlyInTarget().isEmpty() || !diffInfo.getRecordsOnlyInSource().isEmpty()) {
                        registry.registerDiffInfo(diffInfo);
                    }
